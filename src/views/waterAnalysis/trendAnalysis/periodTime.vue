<template>
  <basic-container>
    ...
    <basic-wrapper>
        ...
        <div class="right-info">
          <ul class="map-table--base">
            <li class="base-item">
              ...
              <p class="chart-bottom">{{ sectionInfo.maxData }}</p>
            </li>
          </ul>
          <ul class="map-table--base">
            <li class="base-item">
              <p class="item-top">水质较差</p>
              <p class="chart-bottom">{{ sectionInfo.minData }}</p>
            </li>
          </ul>
        </div>
      ...
      <!-- <div class="right-info" style="display: inline-block; height: 331px; width: 20%;">1</div> -->
    </basic-wrapper>
    <basic-wrapper>
      <!-- <zd-table v-if="query.monitorType === 'auto'" ref="table"></zd-table> -->
      <sg-table :url="'/getSectionWaterQuality'" ref="table">
...
export default {
  ...
  methods: {
    ...
    // 只更新echart数据
    updateEchartData() {
      const url = "/getPointMonitorInfoDbsForSectionList";
      const { region, riverLake } = this.initReginRiverlake();
      const paramObj = {
