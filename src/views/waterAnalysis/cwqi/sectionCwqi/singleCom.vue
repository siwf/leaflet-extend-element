<template>
  <div style="background:#F4F6F8; ">
    ...
    <basic-wrapper>
      ...
      <sg-table ref="table" url="/selectSectionWqiOnce">
...
export default {
  ...
  methods: {
    ...
    initThemeSelectOp() {
      ...
      waterTheme(url, paramObj).then(res => {
        ...
        if (data) {
          ...
          // this.query.exceedFlag = data.exceedFlag.value.map(v => v.value);
    ...
    refreshDetail() {
      ...
      if (choiceStart=='' && choiceEnd=='') {
        let paramObj = {
          ...
          "sectionCodes": this.query.sectionCodes,
    ...
    // 单次wqi柱状图
      barChartData(){
        const url = '/selectSectionWqiOnceBarGraph'
        let choiceEnd = this.query.timeVal[1]
        let choiceStart = this.query.timeVal[0]
        if (choiceStart=='' && choiceEnd=='') {
          let params = {
    ...
    barCharts (chartData) {
      ...
      const xData = chartData[0].sectionWqi.map((v) => v.date);
      const legendData = [];
      const seriesData = chartData.map((v, index) => {
        legendData.push(v.sectionName);
        v.sectionWqi.forEach(item => {
