...
<!--      </div>-->
    ...
    <basic-wrapper>
      ...
      <sg-table ref="table" url="/selectCityWqiTotal">
    ...
    initThemeSelectOp() {
      ...
      waterTheme(url, paramObj).then(res => {
        ...
        if (res.content) {
          ...
          // this.query.exceedFlag = data.sectionType.value.map(v => v.value);
          data.sectionType.value.length = 2;
          data.sectionType.value.map( (e)=> {
            this.query.exceedFlag.push(e.value)
          })
