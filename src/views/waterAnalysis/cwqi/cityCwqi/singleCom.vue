<template>
  <div style="background:#F4F6F8; ">
    ...
    <basic-wrapper>
      ...
      <sg-table ref="table" url="/selectCityWqiOnce">
...
export default {
  ...
  methods: {
    ...
    initReginRiverlake() {
      ...
      // 河流湖库和行政区是互斥的
      // debugger
      if (this.radio === 'region') {      // 湖库
        region = this.query.region;
      }
      if (this.radio === 'river') {     // 行政
    ...
    barChartData(data){
      const xData = data.cityWqiList.map( (e) => e.date);
      const yData = []
      data.cityWqiList.forEach(item => {
        yData.push(item.cityWqi)
      })
      this.chartOp = {
  ...
  mounted() {
        // console.log(dayjs('2021-03').subtract(6,'month').format('YYYY-MM'))
  },
  watch: {
