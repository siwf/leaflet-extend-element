<template>
  <div class="map-popup">
    ...
    <template v-show="baseinfoObj.updateTime">
      <ChartMap chart-id="sdz" :chart-op="chartOp" :style="{ width: '100%', height: '310px' }" />
    </template>
  </div>
</template>

<script>
import { getPointDetailInfoSdz } from "@/api/waterMap";
...
export default {
  ...
  methods: {
    ...
    updateEchart() {
      ...
      getPointDetailInfoSdz(this.query.staCode).then(res => {
        if (res.content) {
        ...
        if (res.content) {
          ...
          const seriesData2 = [];
          const seriesData3 = [];
          const waterType = data.bzll;
          data.data.map(v => {
            xData.push(v.date);

            seriesData1.push(v.total);
            seriesData2.push(v.zmll);
            seriesData3.push(v.jzll);
          });
