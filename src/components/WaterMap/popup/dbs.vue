<template>
  <div class="map-popup">
    ...
    <template v-show="baseinfoObj.updateTime">
      ...
      <ChartMap chart-id="dbs" :chart-op="chartOp" :style="{ width: '100%', height: '231px' }" />
    </template>
  </div>
</template>
...
import { getPointDetailInfoDbs, getPointMonitorInfoDbs } from "@/api/waterMap";
...
export default {
  ...
  methods: {
    ...
    initPopupData() {
      ...
      getPointDetailInfoDbs(this.query.staCode).then(res => {
        if (res.content) {
    ...
    updateEchart() {
      ...
      getPointMonitorInfoDbs({ ...query }).then(res => {
        if (res.content) {
          const data = res.content;
          let xData = [];
          let yData = [];
